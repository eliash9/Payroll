<x-app-layout>
                ...
                <div x-show="step === 1" class="space-y-6">
                    ...
                    <div class="grid md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input name="full_name" class="w-full border-slate-300 rounded-lg focus:ring-blue-500 focus:border-blue-500" required value="{{ old('full_name') }}">
                            @error('full_name') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1">Nama Panggilan (Nickname)</label>
                            <input name="nickname" class="w-full border-slate-300 rounded-lg focus:ring-blue-500 focus:border-blue-500" value="{{ old('nickname') }}">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1">NIK (KTP)</label>
                            <input name="national_id_number" class="w-full border-slate-300 rounded-lg focus:ring-blue-500 focus:border-blue-500" value="{{ old('national_id_number') }}">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1">No. Kartu Keluarga</label>
                            <input name="family_card_number" class="w-full border-slate-300 rounded-lg focus:ring-blue-500 focus:border-blue-500" value="{{ old('family_card_number') }}">
                        </div>
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-medium text-slate-700 mb-1">Tempat Lahir</label>
                                <input name="birth_place" class="w-full border-slate-300 rounded-lg focus:ring-blue-500 focus:border-blue-500" value="{{ old('birth_place') }}">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-slate-700 mb-1">Tanggal Lahir</label>
                                <input name="birth_date" type="date" class="w-full border-slate-300 rounded-lg focus:ring-blue-500 focus:border-blue-500" value="{{ old('birth_date') }}">
                            </div>
                        </div>
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                            ...
                            <div>
                                ...
                                <select name="marital_status" class="w-full border-slate-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
                                    <option value="">Pilih</option>
                                    <option value="single" @selected(old('marital_status') == 'single')>Lajang</option>
                                    <option value="married" @selected(old('marital_status') == 'married')>Menikah</option>
                                    <option value="divorced" @selected(old('marital_status') == 'divorced')>Cerai Hidup</option>
                                    <option value="widowed" @selected(old('marital_status') == 'widowed')>Cerai Mati</option>
                                </select>
                            </div>
                        </div>
                    </div>
                ...
                <div x-show="step === 2" class="space-y-6" style="display: none;">
                    ...
                    <div class="grid md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input name="email" type="email" class="w-full border-slate-300 rounded-lg focus:ring-blue-500 focus:border-blue-500" value="{{ old('email') }}">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1">No. HP / WhatsApp</label>
                            <input name="phone" class="w-full border-slate-300 rounded-lg focus:ring-blue-500 focus:border-blue-500" value="{{ old('phone') }}">
                        </div>
                        <div class="md:col-span-2">
                            <label class="block text-sm font-medium text-slate-700 mb-1">Alamat Lengkap (KTP)</label>
                            <textarea name="address" rows="3" class="w-full border-slate-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">{{ old('address') }}</textarea>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="bg-blue-50 p-4 rounded-lg border border-blue-100 mb-4">
                        <div class="grid md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input name="employee_code" class="w-full border-slate-300 rounded-lg focus:ring-blue-500 focus:border-blue-500" required value="{{ old('employee_code') }}">
                        ...
                        <div>
                            ...
                            <select name="employment_type" class="w-full border-slate-300 rounded-lg focus:ring-blue-500 focus:border-blue-500" required>
                                @foreach(['permanent'=>'Permanent','contract'=>'Contract','intern'=>'Intern','outsourcing'=>'Outsourcing'] as $val=>$label)
                                    <option value="{{ $val }}" @selected(old('employment_type',$val)===$val)>{{ $label }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1">Tanggal Bergabung</label>
                            <input name="join_date" type="date" class="w-full border-slate-300 rounded-lg focus:ring-blue-500 focus:border-blue-500" value="{{ old('join_date') }}">
                        </div>
                    </div>
                    ...
                    <div class="grid md:grid-cols-3 gap-6 pt-4 border-t">
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input name="basic_salary" type="number" step="0.01" class="w-full pl-10 border-slate-300 rounded-lg focus:ring-blue-500 focus:border-blue-500" value="{{ old('basic_salary') }}">
                            </div>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1">Hourly Rate (Relawan)</label>
                            <input name="hourly_rate" type="number" step="0.01" class="w-full border-slate-300 rounded-lg focus:ring-blue-500 focus:border-blue-500" value="{{ old('hourly_rate') }}">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1">Komisi %</label>
                            <input name="commission_rate" type="number" step="0.01" class="w-full border-slate-300 rounded-lg focus:ring-blue-500 focus:border-blue-500" value="{{ old('commission_rate') }}">
                        </div>
                    </div>

                    <div class="flex items-center gap-2 pt-4">
                        <input type="checkbox" name="is_volunteer" value="1" id="is_volunteer" @checked(old('is_volunteer')) class="rounded border-slate-300 text-blue-600 focus:ring-blue-500">
    ...
    <script>
        ...
        async function loadOptions(endpoint, selectEl) {
        ...
        if (companySelect?.value) { onCompanyChange(); }
    </script>
    @endpush
</x-app-layout>
