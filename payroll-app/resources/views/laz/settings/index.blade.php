        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <form method="POST" action="{{ route('laz.settings.update') }}">
                    ...
                    <div class="mb-6">
                        <h3 class="text-lg font-medium text-gray-900 mb-4">Konfigurasi WhatsApp</h3>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700">WhatsApp API URL</label>
                                <input type="url" name="whatsapp_api_url" value="{{ old('whatsapp_api_url', $settings['whatsapp_api_url'] ?? '') }}" placeholder="https://api.whatsapp-service.com/send" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">WhatsApp API Key / Token</label>
                                <input type="text" name="whatsapp_api_key" value="{{ old('whatsapp_api_key', $settings['whatsapp_api_key'] ?? '') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                            </div>
                        </div>
                    </div>
                ...
                <div class="mb-6">
                    <h3 class="text-lg font-medium text-gray-900 mb-4">Test Kirim Notifikasi</h3>
                    <p class="text-sm text-gray-500 mb-4">Gunakan form ini untuk memastikan konfigurasi email dan whatsapp berjalan dengan baik.</p>
                    
                    <form method="POST" action="{{ route('laz.settings.test') }}">
                        @csrf
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Email Tujuan</label>
                                <input type="email" name="test_email" placeholder="contoh@email.com" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">WhatsApp Tujuan</label>
                                <input type="text" name="test_whatsapp" placeholder="08123xxx" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                            </div>
                            <div>
                                <button type="submit" class="w-full bg-green-600 text-white px-4 py-2 rounded-md hover:bg-green-700">Kirim Test</button>
                            </div>
                        </div>
                    </form>
                </div>
