<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Tambah Job Profile</h2>
    </x-slot>
    ...
    <div class="py-8 max-w-5xl mx-auto" x-data="jobForm()">
        <form method="post" action="{{ route('jobs.store') }}" class="space-y-6">
            ...
            <div class="bg-white shadow-sm rounded p-6">
                ...
                <div class="grid md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input name="title" class="w-full border rounded px-3 py-2" required value="{{ old('title') }}" placeholder="Contoh: Staff IT">
                    </div>
                    <div>
                        <label class="text-sm">Kode</label>
                        <input name="code" class="w-full border rounded px-3 py-2" value="{{ old('code') }}" placeholder="Contoh: JOB-IT-01">
                    </div>
                    <div class="md:col-span-2">
                        <label class="text-sm">Deskripsi Singkat</label>
                        <textarea name="description" class="w-full border rounded px-3 py-2" placeholder="Deskripsi singkat tentang pekerjaan ini...">{{ old('description') }}</textarea>
                    </div>
                </div>
            </div>
                ...
                <template x-for="(resp, index) in responsibilities" :key="index">
                    <div class="grid grid-cols-12 gap-2 mb-2 items-start">
                        ...
                        <div class="col-span-10">
                            <textarea :name="'responsibilities['+index+'][responsibility]'" x-model="resp.text" class="w-full border rounded px-3 py-2 text-sm" rows="2" placeholder="Deskripsikan tanggung jawab..."></textarea>
                        </div>
                        <div class="col-span-1 flex justify-center pt-2">
                ...
                <template x-for="(req, index) in requirements" :key="index">
                    <div class="grid grid-cols-12 gap-2 mb-2 items-start">
                        ...
                        <div class="col-span-8">
                            <textarea :name="'requirements['+index+'][requirement]'" x-model="req.text" class="w-full border rounded px-3 py-2 text-sm" rows="2" placeholder="Contoh: Minimal S1 Teknik Informatika..."></textarea>
                        </div>
                        <div class="col-span-1 flex justify-center pt-2">
            ...
            <div class="flex gap-2">
                <button class="bg-blue-600 text-white px-6 py-2 rounded shadow hover:bg-blue-700">Simpan Profil Pekerjaan</button>
