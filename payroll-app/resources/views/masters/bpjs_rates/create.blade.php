            ...
            <form method="post" action="{{ route('bpjs-rates.store') }}" class="space-y-4">
                ...
                <div class="grid md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input name="employer_rate" type="number" step="0.01" min="0" class="w-full border rounded px-3 py-2" required value="{{ old('employer_rate') }}">
                    </div>
                    <div>
                        <label class="text-sm">Persen Karyawan</label>
                        <input name="employee_rate" type="number" step="0.01" min="0" class="w-full border rounded px-3 py-2" required value="{{ old('employee_rate') }}">
                    </div>
                    <div>
                        <label class="text-sm">Cap Min</label>
                        <input name="salary_cap_min" type="number" step="0.01" min="0" class="w-full border rounded px-3 py-2" value="{{ old('salary_cap_min') }}">
                    </div>
                    <div>
                        <label class="text-sm">Cap Max</label>
                        <input name="salary_cap_max" type="number" step="0.01" min="0" class="w-full border rounded px-3 py-2" value="{{ old('salary_cap_max') }}">
                    </div>
                    <div>
